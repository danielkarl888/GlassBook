...
function BookDetails({ id }) {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            if (!response.ok) {
                setComments([{book_name: "error with server - please try again!", author_name: "error",publisher:"error", length:"error" }])
            } else {
                const json = await response.json();
                setComments(json);
                ...
                <div className="col-6">
                    <div>
                        {comments ? (<div><h2> {comments[0].book_name}</h2>
                            <img className="rounded img-fluidrounded mx-auto d-block" src={comments[0].img} alt="Error" width="20%"  ></img>
